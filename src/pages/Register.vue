...
function useRegister() {
  ...
  async function handleSubmit() {
    ...
    if (response.error) {
      ...
      setFeedbackMessage({
        type: 'error',
        message: `Error al crear la cuenta del usuario. (${response.error})`,
      })
      return;
    }
...
<template>
  <h1 class="text-4xl mb-4">Crear Cuenta</h1>

  <form action="#" method="post" @submit.prevent="handleSubmit">
    <V_Label for="email">Usuario:</V_Label>
    <V_Input
        type="email"
        name="email"
        id="email"
    ...
    <V_Button full class="flex items-center justify-center">
